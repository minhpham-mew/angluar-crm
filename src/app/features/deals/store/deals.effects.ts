    ...
    this.actions$.pipe(
      ...
      exhaustMap(([action, tenantId]) => {
        ...
        return this.appwriteService
          .getDeals(tenantId)
          .then((deals) => DealsActions.loadDealsSuccess({ deals }))
          .catch((error) => DealsActions.loadDealsFailure({ error: error.message }));
      }),
    ),
  );
    ...
    this.actions$.pipe(
      ...
      exhaustMap(({ deal }) =>
        this.appwriteService
          .createDeal(deal)
          .then((newDeal) => DealsActions.createDealSuccess({ deal: newDeal }))
          .catch((error) => DealsActions.createDealFailure({ error: error.message })),
      ),
    ),
  );
    ...
    this.actions$.pipe(
      ...
      exhaustMap(({ id, changes }) =>
        this.appwriteService
          ...
          .then((deal) => DealsActions.updateDealSuccess({ deal }))
          .catch((error) => DealsActions.updateDealFailure({ error: error.message })),
      ),
    ),
  );
  ...
  deleteDeal$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap(({ id }) =>
        this.appwriteService
          .deleteDeal(id)
          .then(() => DealsActions.deleteDealSuccess({ id }))
          .catch((error) => DealsActions.deleteDealFailure({ error: error.message })),
      ),
    ),
  );
