...
export class AuthEffects {
  ...
  login$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap(({ email, password }) =>
        this.appwriteService.login(email, password).then(async (session) => {
  ...
  loadCurrentUser$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap(() =>
        this.appwriteService.getCurrentUser().then(async (appwriteUser) => {
          const prefs = await this.appwriteService.getUserPrefs() as unknown as UserPrefs;
  ...
  signup$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap(({ name, email, password, companyName }) =>
        this.appwriteService.signup(name, email, password, companyName).then(async (appwriteUser) => {
          const prefs = await this.appwriteService.getUserPrefs() as unknown as UserPrefs;
  ...
  signupSuccess$ = createEffect(() =>
    this.actions$.pipe(
      ...
      tap(() => {
        this.router.navigate(['/dashboard']);
        this.store.dispatch(NotificationsActions.addNotification({
          notification: { message: 'Account created successfully! Welcome to your CRM!', type: 'success' }
        }));
      })
    ),
    { dispatch: false }
  ...
  checkSession$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap(() =>
        this.appwriteService.hasActiveSession().then(async (hasSession) => {
          if (hasSession) {
            try {
              const appwriteUser = await this.appwriteService.getCurrentUser();
              const prefs = await this.appwriteService.getUserPrefs() as unknown as UserPrefs;
  ...
  sessionExpired$ = createEffect(() =>
    this.actions$.pipe(
      ...
      tap(() => {
        this.router.navigate(['/login']);
        this.store.dispatch(NotificationsActions.addNotification({
          notification: { message: 'Your session has expired. Please sign in again.', type: 'warning' }
        }));
      })
    ),
    { dispatch: false }
